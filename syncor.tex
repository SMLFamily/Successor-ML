...
The {\sl formatting characters}\index{6.3} are a subset of the non-printable
            characters including at least space, tab, newline, form feed\ADD{, vertical tab, and carriage return}.
...
TyVar, the {\sl equality} type variables, consists of those \REPL{which}
...
identifier $\vid$ \REPL{that}{which} has infix status\ADD{ in an expression or pattern}; elsewhere {\OP}, where
...
\item[\textcolor{\addcolor}{$\bullet$}]
      \ADD{Any $\tyvar$ occurring on the right-hand side of a $\typbind$ or $\datbind$ of the form
