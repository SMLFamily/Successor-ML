...
\item[\textcolor{\addcolor}{$\bullet$}]
	\ADD{A declaration \dec\ appearing in a \strdec\ may not be a sequential or local declaration.}
\item[\textcolor{\addcolor}{$\bullet$}]
	\ADD{In a sequential specification ``$\seqspec$,'' $\spec_2$ may not contain a sharing specification.}
...
\item[\textcolor{\addcolor}{$\bullet$}]
	\ADD{No $\topdec$ may contain, as an initial segment, a $\strdec$ followed by a semicolon.
	Furthermore, the \strdec\ man not be a sequential declaration ``\seqdec.''}
\end{itemize}
